...
class Ion_auth_model extends CI_Model
{
	...
	public function login($identity, $password, $remember=FALSE)
	{
		...
		$query = $this->db->select($this->identity_column . ', email, user_id, id, password, active, last_login')
						  ->where($this->identity_column, $identity)
						  ->limit(1)
						  ->order_by('id', 'desc')
						  ->get($this->tables['users']);
    ...
    public function fetch_column_value($table, $column_name, $where_array){
     ...
     public function user_list()
     {
     	 ...
     	 ($user_id == 1) ? $cond = [] : $cond = ['users.user_id' => $user_id];
     	  $query=$this->db->select('users.id , users.first_name, users.image,users.last_name,users.company , users.email ,users.active,users.created_on ,team_role.role_name')
     	       ->where($cond)
	            ->from('users')
	 ...
	 return $query->result();
      ...
      public function workroom_user_list()
     {
     	  $query=$this->db->select('users.id , users.first_name, users.image,users.last_name,users.company , users.email ,users.active,users.created_on ,team_role.role_name')
     	       //->where('id',$id)
	            ->from('users')
           ...
           public function team_list()
     {
     	 $user_id = $this->session->userdata('user_id');
     	 ($user_id == 1) ? $cond = [] : $cond = ['users.user_id' => $user_id];
     	  $query=$this->db->select('users.id , users.first_name, users.last_name,users.company , users.email ,users.active,users.created_on ,team_role.role_name,project.project_name')
     	       ->where($cond)
	            ->from('users')
	            ->join('team_role','users.team_role_id = team_role.id')
...
public function w_user_list()
{
$user_id = $this->session->userdata('user_id');
//echo $user_id; exit();
$query=$this->db->select('users.id,team_role.role_name')
->from('users')
...
return $query->result();
   ...
   public function find_team($id){
     ...
     public function find_role()
       {
       	  //$user_id = $this->session->userdata('user_id');
    // ($user_id == 1) ? $cond = [] : $cond = ['team_role.user_id' => $user_id];
          $query=$this->db->select('*')
                       ->order_by('id','desc')
             // ->where($cond)
                       ->get('team_role');
