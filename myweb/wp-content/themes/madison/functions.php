...
function madison_scripts() {
    ...
    if (in_array('administrator', (array) $user->roles)) {
        ...
        $frontend_variables['edit_button'] = esc_html__('Edit', 'madison');
        ...
        if (function_exists('get_redux_instance')) {
            ...
            $frontend_variables['edit_links']['configure_templates'] = array(
                ...
                'text' => esc_html__('Configure template', 'madison'),
                'target' => '_blank',
            );
...
add_action('wp_enqueue_scripts', 'madison_styles');

function madison_styles() {
    global $azexo_azh;
    ...
    if (isset($options['google_font_families']) && is_array($options['google_font_families'])) {
        ...
        foreach ($options['google_font_families'] as $font_family) {
            ...
            if (!empty($font_family)) {
                if ('off' !== esc_html_x('on', $font[0] . ' font: on or off', 'madison')) {
