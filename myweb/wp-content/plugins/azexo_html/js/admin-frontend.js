(function($) {
    ...
    azh.style_in_page = function(css, verbose) {
        ...
        for (var i = 0; i < L; i++) {
            ...
            if (who.style) {
                ...
                var val_before = getComputedStyle(who, ':before')[css];
                if (val_before) {
                    if (verbose)
                        values.push([hoo, val_before]);
                    else if (values.indexOf(val_before) == -1)
                        values.push(val_before);
                }
                var val_after = getComputedStyle(who, ':after')[css];
                if (val_after) {
                    if (verbose)
                        values.push([hoo, val_after]);
    ...
    azh.fixed_inner_html = '.az-gmap, iframe';
    ...
    azh.extract_html = function($wrapper) {
        var html = '';
        if ($wrapper.data('azh-fixed-inner-html') !== undefined) {
            html = html + $wrapper.data('azh-fixed-inner-html');
        } else {
    ...
    azh.refresh_controls = function($wrapper) {
        var $controls_container = false;
        if ($('.azh-controls-container').length) {
            $controls_container = $('.azh-controls-container');
            $controls_container.detach();
        } else {
            $controls_container = $('<div class="azh-controls-container"></div>');
        }
        ...
        $controls_container.appendTo($body);
    };
    ...
    azh.customization_init = function($wrapper) {
        ...
        function open_dropdown_modal($element) {
            var $modal = $('<div class="azh-modal"></div>');
            $('<div class="azh-modal-title">' + azh.fi18n.dropdown_options_edit + '</div>').appendTo($modal);
            $('<div class="azh-modal-label">' + azh.fi18n.options + '</div>').appendTo($modal);

            var $table = $('<div class="azh-table azh-dropdown-options"></div>').appendTo($modal);
            var $row = $('<div class="azh-row azh-head"><div class="azh-cell">' + azh.fi18n.label + '</div><div class="azh-cell">' + azh.fi18n.value + '</div><div class="azh-cell">' + azh.fi18n.clone + '</div><div class="azh-cell">' + azh.fi18n.remove + '</div></div>').appendTo($table);
            $element.children().each(function() {
                var $option = $(this);
                var $row = $('<div class="azh-row"></div>').appendTo($table);
                $row.data('option', $option);
                $('<div class="azh-cell azh-label" contenteditable="true">' + $option.text() + '</div>').appendTo($row).on('blur keyup paste input', function() {
                    var $option = $(this).closest('.azh-row').data('option');
                    $option.text($(this).text());
                });
                $('<div class="azh-cell azh-value" contenteditable="true">' + $option.val() + '</div>').appendTo($row).on('blur keyup paste input', function() {
                    var $option = $(this).closest('.azh-row').data('option');
                    $option.val($(this).text());
                    azh.set_stored_attribute($option, 'value', $option.val());
                });
                $('<div class="azh-cell azh-clone"></div>').appendTo($row).on('click', function() {
                    var $row = $(this).closest('.azh-row');
                    var $new_row = $row.clone(true);
                    $new_row.insertAfter($row);
                    var $new_option = $row.data('option').clone(true);
                    $new_option.insertAfter($row.data('option'));
                    $new_row.data('option', $new_option);
                });
                $('<div class="azh-cell azh-remove"></div>').appendTo($row).on('click', function() {
                    var $row = $(this).closest('.azh-row');
                    $row.data('option').remove();
                    $row.remove();
                });
            });
        ...
        function grid_editor($grid) {
            ...
            $grid.children().each(function() {
                var $column = $(this);
                if ($prev_column) {
                    var $resizer = $('<div class="azh-width-resizer"></div>').appendTo($controls_container);
        ...
        function enable_contenteditable($element) {
            if ($element.attr('contenteditable') !== 'true' && $element.find('[contenteditable="true"]').length === 0 && $element.find('[draggable="true"]').length === 0 && $element.parents('[contenteditable="true"]').length === 0) {
                ...
                $element.on('focus click', function(event) {
                    ...
                    var $this = $(this);
                    if (['block', 'table-cell'].indexOf($this.css('display')) >= 0) {
        ...
        function control_create($element, options) {
            ...
            function color_attribute_change() {
                var $control = $(this).closest('.azh-control');
                var options = $control.data('azh-options');
                var $element = $control.data('azh-linked-element');
                var color = $control.find('input[type="color"]').val();
                if ('alpha' in options && options['alpha']) {
                    var rgb = hex2rgb(color);
                    var alpha = $control.find('input[type="number"]').val() / 100;
                    color = 'rgba(' + rgb[0] + ',' + rgb[1] + ',' + rgb[2] + ',' + alpha + ')';
                }
                $element.attr(options['attribute'], color);
                azh.set_stored_attribute($element, options['attribute'], color);
            ...
            if (!exists) {
                ...
                switch (options['type']) {
                    ...
                    case 'integer-attribute':
                        $('<label>' + options['control_text'] + '</label>').appendTo($control);
                        $('<input type="number" step="' + ('step' in options ? options['step'] : '1') + '"/>').appendTo($control).on('change', function() {
                            var $this = $(this);
                            var $control = $this.closest('.azh-control');
                            var $element = $control.data('azh-linked-element');
                    ...
                    case 'color-attribute':
                        ...
                        if ('alpha' in options && options['alpha']) {
                            ...
                            }).on('change', color_attribute_change);
                        }
                        $('<input type="color"/>').appendTo($control).on('change', color_attribute_change);
                        $control.on('azh-init', function() {
                            var $this = $(this);
                            var $element = $this.data('azh-linked-element');
                            var color = $element.attr($this.data('azh-options')['attribute']);
                            if (hexToRgbA(color, 1)) {
                    ...
                    case 'font-family':
                        ...
                        $control.on('azh-init', function() {
                            ...
                            var $element = $this.data('azh-linked-element');
                            var fonts = [];
                            var font_families = azh.style_in_page('fontFamily');
                            $(font_families).each(function() {
                                fonts = fonts.concat(this.split(','));
                            });
                            fonts = $.unique(fonts);
                            $(fonts).each(function() {
                                $dropdown.append('<option value="' + this + '">' + this + '</option>');
                            });
                            $dropdown.val($element.css($this.data('azh-options')['property']));
                        });
                        break;
                    case 'background-image':
                        ...
                        while ((url = pattern.exec($element.attr('style'))) != null) {
                            $('<img src="' + (url ? url[1] : azh.no_image) + '" alt="' + azh.fi18n.select_image + '"/>').appendTo($control).on('contextmenu', function(event) {
                                event.preventDefault();
                                var $this = $(this);
                                var $control = $this.closest('.azh-control');
                                var $element = $control.data('azh-linked-element');

                                azh.set_stored_style($element, 'background-image', azh.get_stored_style($element, 'background-image').replace((($this.attr('src') !== azh.no_image) ? $this.attr('src') : '/'), '/'));
                                $element.attr('style', azh.get_stored_attribute($element, 'style'));

                                $this.attr('src', azh.no_image);
                                ...
                                azh.open_image_select_dialog(event, function(url, id) {
                                    ...
                                    azh.set_stored_style($element, 'background-image', azh.get_stored_style($element, 'background-image').replace((($this.attr('src') !== azh.no_image) ? $this.attr('src') : '/'), url));
                                    $element.attr('style', azh.get_stored_attribute($element, 'style'));
                                    $this.attr('src', url);
                        ...
                        $control.on('azh-init', function() {
                            ...
                            while ((url = pattern.exec($element.attr('style'))) != null) {
                                if (url[1] === '' || url[1] === '/') {
                                    $image.clone(true).appendTo($this).attr('src', azh.no_image);
                                } else {
                    ...
                    case 'image-attribute':
                        ...
                        $('<img src="' + (url ? url : azh.no_image) + '" alt="' + azh.fi18n.select_image + '"/>').appendTo($control).on('contextmenu', function(event) {
                            event.preventDefault();
                            var $this = $(this);
                            var $control = $this.closest('.azh-control');
                            var $element = $control.data('azh-linked-element');
                        ...
                        $control.on('azh-init', function() {
                            ...
                            if (url) {
                                ...
                                $this.find('img').attr('src', azh.no_image);
                            }
                        });
                        break;
                    case 'exists-class':
        ...
        function make_element_empty($element) {
            if ($element.is('[data-element*="shortcode"]')) {
                $element.off('mouseenter.azh-shortcode').off('mouseleave.azh-shortcode');
            }
            $element.attr('data-element', '');
            azh.set_stored_attribute($element, 'data-element', '');
            if ($element.attr('contenteditable')) {
                $element.attr('contenteditable', 'false');
                $element.removeClass('az-contenteditable');
                azh.remove_from_stored_classes($element, 'az-contenteditable');
            }
            $element.empty();
        }
        function create_element_controls($element) {
            var $element_controls = $('<div class="azh-element-controls"></div>').appendTo($controls_container);
        ...
        function add_element_button($element_controls, after) {
            return $('<div class="azh-add-element-' + (after ? 'after' : 'before') + '" title="' + (after ? azh.fi18n.add_element_after : azh.fi18n.add_element_before) + '"></div>').appendTo($element_controls).on('click', function() {
                var $button = $(this);
                var $element = $button.data('azh-linked-element');
                var $element_wrapper = $element.closest('.az-element-wrapper');
                var $new_element_wrapper = false;
                if ($element_wrapper.is('[data-element]')) {
                    if ($element_wrapper.contents().length === 0) {
                        ...
                        if (after) {
                            ...
                            $new_element_wrapper.insertBefore($element_wrapper);
                        }
                    }
                } else {
                ...
                make_element_empty($new_element);
        ...
        function has_stored_css_property($element, property) {
            var property_pattern = new RegExp('(^' + property + '|[ \\"\\\'\\;]' + property + ')\\s*:', 'i');
            if ($element.attr('style').match(property_pattern)) {
                var style = azh.get_stored_attribute($element, 'style');
                if (style !== false) {
        ...
        $wrapper.on('contextmenu', function(event) {
            ...
            if ($context_menu.children().length) {
                ...
                $context_menu.show();
                $context_menu.find('.azh-modal-button, .azh-button').on('click', function() {
                    $context_menu.hide();
                });
        ...
        $wrapper.find('span.az-inline').each(function() {
            var $this = $(this);
            if ($this.parent().children().length === $this.parent().children().filter('span.az-inline').length) {
        ...
        $wrapper.find('select').each(function() {
            var $select = $(this);
            var $button = $('<div class="azh-modal-button" data-type="azh-select" title="' + azh.fi18n.dropdown_options_edit + '">' + azh.fi18n.dropdown_options_edit + '</div>').on('click', function(event) {
                var $this = $(this);
                var $select = $this.data('azh-linked-element');
                open_dropdown_modal($select);
            });
            var context = $select.data('azh-context');
            if (!context) {
                context = [];
            }
            context.push($button);
            $select.data('azh-context', context);
            $select.addClass('azh-context');
        });
        ...
        $wrapper.find('[data-element]').each(function() {
            ...
            if ($element.contents().length && ($element.attr('data-element') === '' || $element.attr('data-element') === ' ')) {
            ...
            $element.on('mouseenter', function(event) {
                ...
                var $element_controls = $(this).data('azh-controls');
                setTimeout(function() {
                    if (!$element_controls.is(':hover') && $('.azh-resizer:hover').length === 0 && $('.azh-shortcode:hover').length === 0) {
                        $element_controls.hide();
                    } else {
                ...
                if ($element.contents().length === 0) {
                    ...
                    azh.show_elements_dialog(function(path, html) {
                        ...
                        var $button = remove_element_button($element.data('azh-controls'));
                        $button.data('azh-linked-element', $element);
                        $button = add_element_button($element.data('azh-controls'), true);
                        $button.data('azh-linked-element', $element);
                        $button = add_element_button($element.data('azh-controls'), false);
                        $button.data('azh-linked-element', $element);
        ...
        $wrapper.find('[data-shortcode], [data-element*="shortcode"]').each(function() {
            var $shortcode = $(this);
            $shortcode.attr('title', azh.fi18n.click_to_edit_shortcode);
            $shortcode.on('mouseenter.azh-shortcode', function(event) {
                var $shortcode = $(this);
                var $hover = $('<div class="azh-shortcode"></div>').appendTo($controls_container).on('click', function() {
            ...
            }).on('mouseleave.azh-shortcode', function(event) {
        ...
        $wrapper.find('[data-cloneable], [data-cloneable-inline]').each(function() {
            ...
            var $cloneable = $(this);
            var $cloneable_controls = $('<div class="azh-cloneable-controls"></div>').appendTo($controls_container);
            ...
            $cloneable.children().each(function() {
                ...
                var $button = $('<div class="azh-button azh-clone" data-type="azh-clone">' + azh.fi18n.clone + '</div>').on('click', function(event) {
                    ...
                    $new_element.find('[data-shortcode]').removeAttr('data-shortcode');
                            ...
                            if (old_id && old_id in linked_ids && linked_ids[old_id]) {
                                $(linked_ids[old_id]).each(function() {
                                    ...
                                    if (!$this.is($old_handle_id)) {
                                        ...
                                        $new_linked_element.find('[data-shortcode]').removeAttr('data-shortcode');
            ...
            if (elements_list || $cloneable.is('.az-elements-list')) {
                ...
                var $add_button = $('<div class="azh-add-element" title="' + azh.fi18n.add_element + '"></div>').appendTo($cloneable_controls).on('click', function() {
                    ...
                    var $element_wrapper = $cloneable.find('.az-element-wrapper').last();
                    var $new_element_wrapper = false;
                    if ($element_wrapper.is('[data-element]')) {
        ...
        $wrapper.find('[data-isotope-items]').each(function() {
            ...
            $isotope.children().each(function() {
                var $item = $(this);
                var $item_controls = $('<div class="azh-item-controls"></div>').appendTo($controls_container);
                $item_controls.hide();
        ...
        $wrapper.find('.az-resize[style*="height"], .az-resize[style*="padding-top"], .az-resize[style*="padding-bottom"]').each(function() {
            $(this).on('mouseenter', function(event) {
                var $element = $(this);
                if (has_stored_css_property($element, 'padding-top') && !$element.data('azh-padding-top-resizer')) {
                    var $resizer = $('<div class="azh-resizer">' + azh.get_stored_style($element, 'padding-top') + '</div>').appendTo($controls_container).on('mousedown', function(event) {
                        var $resizer = $(this);
                        $resizer.data('azh-drag', true);
                        $resizer.data('azh-drag-y', event.pageY);
                        $resizer.css({
                            "top": $element.offset().top,
                            "height": parseInt(azh.get_stored_style($element, 'padding-top'), 10)
                        });
                        $document.off('mouseup.resizer').on('mouseup.resizer', function(e) {
                            $resizer.data('azh-drag', false);
                            if (!$resizer.is(':hover')) {
                                $document.off('mouseup.resizer');
                            }
                        });
                        $document.off('mousemove.resizer').on('mousemove.resizer', function(e) {
                            if ($resizer.data('azh-drag')) {
                                var d = e.pageY - $resizer.data('azh-drag-y');
                                var v = (parseInt(azh.get_stored_style($element, 'padding-top'), 10) + d);
                                if (!isNaN(v)) {
                                    if (v < 0) {
                                        v = 0;
                                    }
                                    v = v + 'px';
                                    azh.set_stored_style($element, 'padding-top', v);
                                    $element.css('padding-top', v);
                                    $resizer.text(v);
                                    $resizer.data('azh-drag-y', e.pageY);
                                    $resizer.height(v);
                                }
                            }
                        });
                        ...
                        var $resizer = $(this);
                        if (!$resizer.data('azh-drag')) {
                            $element.data('azh-padding-top-resizer', false);
                            $resizer.remove();
                        ...
                        "height": parseInt(azh.get_stored_style($element, 'padding-top'), 10) > 10 ? parseInt(azh.get_stored_style($element, 'padding-top'), 10) : 10
                    });
                    $element.data('azh-padding-top-resizer', $resizer);
                }
                if (has_stored_css_property($element, 'padding-bottom') && !$element.data('azh-padding-bottom-resizer')) {
                    var $resizer = $('<div class="azh-resizer">' + azh.get_stored_style($element, 'padding-bottom') + '</div>').appendTo($controls_container).on('mousedown', function(event) {
                        var $resizer = $(this);
                        $resizer.data('azh-drag', true);
                        $resizer.data('azh-drag-y', event.pageY);
                        $resizer.css({
                            "top": $element.offset().top + $element.innerHeight() - parseInt(azh.get_stored_style($element, 'padding-bottom'), 10),
                            "height": parseInt(azh.get_stored_style($element, 'padding-bottom'), 10)
                        });
                        $document.off('mouseup.resizer').on('mouseup.resizer', function(e) {
                            $resizer.data('azh-drag', false);
                            if (!$resizer.is(':hover')) {
                        ...
                        $document.off('mousemove.resizer').on('mousemove.resizer', function(e) {
                            if ($resizer.data('azh-drag')) {
                                var d = e.pageY - $resizer.data('azh-drag-y');
                                var v = (parseInt(azh.get_stored_style($element, 'padding-bottom'), 10) + d);
                                if (!isNaN(v)) {
                                    if (v < 0) {
                                        v = 0;
                                    }
                                    v = v + 'px';
                                    azh.set_stored_style($element, 'padding-bottom', v);
                                    $element.css('padding-bottom', v);
                                    $resizer.text(v);
                                    $resizer.data('azh-drag-y', e.pageY);
                                    $resizer.height(v);
                                }
                        ...
                        var $resizer = $(this);
                        if (!$resizer.data('azh-drag')) {
                            $element.data('azh-padding-bottom-resizer', false);
                            $resizer.remove();
                            $element.trigger('mouseleave');
                        }
                    }).css({
                        "top": $element.offset().top + $element.innerHeight() - (parseInt(azh.get_stored_style($element, 'padding-bottom'), 10) > 0 ? parseInt(azh.get_stored_style($element, 'padding-bottom'), 10) : 10),
                        "left": $element.offset().left,
                        "width": $element.innerWidth(),
                        "height": parseInt(azh.get_stored_style($element, 'padding-bottom'), 10) > 10 ? parseInt(azh.get_stored_style($element, 'padding-bottom'), 10) : 10
                    });
                    $element.data('azh-padding-bottom-resizer', $resizer);
                }
                if (has_stored_css_property($element, 'height') && !$element.data('azh-height-resizer')) {
                    var $resizer = $('<div class="azh-resizer">' + azh.get_stored_style($element, 'height') + '</div>').appendTo($controls_container).on('mousedown', function(event) {
                        var $resizer = $(this);
                        $resizer.data('azh-drag', true);
                        $resizer.data('azh-drag-y', event.pageY);
                        $resizer.css({
                            "height": parseInt(azh.get_stored_style($element, 'height'), 10)
                        });
                        $document.off('mouseup.resizer').on('mouseup.resizer', function(e) {
                            $resizer.data('azh-drag', false);
                            if (!$resizer.is(':hover')) {
                        ...
                        $document.off('mousemove.resizer').on('mousemove.resizer', function(e) {
                            if ($resizer.data('azh-drag')) {
                                var d = e.pageY - $resizer.data('azh-drag-y');
                                var v = (parseInt(azh.get_stored_style($element, 'height'), 10) + d);
                                if (!isNaN(v)) {
                                    if (v < 0) {
                                        v = 0;
                                    }
                                    v = v + 'px';
                                    azh.set_stored_style($element, 'height', v);
                                    $element.css('height', v);
                                    $resizer.text(v);
                                    $resizer.data('azh-drag-y', e.pageY);
                                    $resizer.height(v);
                                }
                        ...
                        var $resizer = $(this);
                        if (!$resizer.data('azh-drag')) {
                            $element.data('azh-height-resizer', false);
                            $resizer.remove();
                            $element.trigger('mouseleave');
                        }
                    }).css({
                        ...
                        "left": $element.offset().left,
                        "width": $element.innerWidth(),
                        "height": parseInt(azh.get_stored_style($element, 'height'), 10) > 10 ? parseInt(azh.get_stored_style($element, 'height'), 10) : 10
                    });
                    $element.data('azh-height-resizer', $resizer);
                }
            }).on('mouseleave', function() {
                setTimeout(function() {
                    var $element = $(this);
                    if ($element.data('azh-padding-top-resizer')) {
                        if (!$element.data('azh-padding-top-resizer').is(':hover') && !$element.data('azh-padding-top-resizer').data('azh-drag')) {
                            $element.data('azh-padding-top-resizer').remove();
                            $element.data('azh-padding-top-resizer', false);
                        }
                    }
                    if ($element.data('azh-padding-bottom-resizer')) {
                        if (!$element.data('azh-padding-bottom-resizer').is(':hover') && !$element.data('azh-padding-bottom-resizer').data('azh-drag')) {
                            $element.data('azh-padding-bottom-resizer').remove();
                            $element.data('azh-padding-bottom-resizer', false);
                        }
                    }
                    if ($element.data('azh-height-resizer')) {
                        if (!$element.data('azh-height-resizer').is(':hover') && !$element.data('azh-height-resizer').data('azh-drag')) {
                            $element.data('azh-height-resizer').remove();
                            $element.data('azh-height-resizer', false);
                        }
                    }
                });
                    ...
                    switch (options['type']) {
                        ...
                        case 'font-family':
                            $wrapper.find('[style]').each(function() {
                                var $this = $(this);
                                if (has_stored_css_property($this, options['property'])) {
                                    control_create($this, options);
                                }
                            });
                            break;
    ...
    azh.section_customization_init = function($wrapper) {
        ...
        $wrapper.on('mouseenter', function(event) {
            ...
            setTimeout(function() {
                if (!$section_controls.is(':hover') && $('.azh-resizer:hover').length == 0) {
                    $section_controls.hide();
                } else {
    ...
    azh.library_init = function($content) {
        ...
        $('.azh-add-section').off('click').on('click', function() {
            ...
            if ($('.azh-library').css('display') == 'none') {
                ...
                $('.azh-sections').height($('#azexo-html-library > .azh-builder').height() - ($('.azh-structure').outerHeight() > 100 ? 100 : $('.azh-structure').outerHeight()) - 120);
        ...
        if ($('#azexo-html-library > .azh-general .azh-section').length === 0) {
            $('#azexo-html-library > .azh-library-actions > .azh-general').remove();
        ...
        $(function() {
            if (window === window.parent) {
                azh.edit_links_refresh = function() {
                    ...
                    var $container = false;
                    if ($('.azh-edit-links').length) {
                        $container = $('.azh-edit-links');
                        $container.detach();
                    } else {
                        $container = $('<div class="azh-edit-links"></div>');
                    }
                        ...
                        for (var i = 0; i < selectors.length; i++) {
                            ...
                            $(selector).each(function() {
                                if (!$(this).data('edit-link-control')) {
                                    var control = $('<div class="azh-edit-link" data-edit-link-control=""><a href="' + azh.edit_links[links_type].links[selector] + '&occurrence=' + occurrence + '" target="' + azh.edit_links[links_type].target + '">' + azh.edit_links[links_type].text + '</a></div>').appendTo($container).css({
                                        "top": "0",
                                        "left": "0",
                                        "width": "0",
                                        "height": "0",
                ...
                $('#wp-admin-bar-edit-links').off('click.azh').on('click.azh', function(event) {
                    ...
                    if ($this.is('.azh-edit-links-active')) {
                        $('.azh-edit-links div.azh-edit-link[style] a[href][style][target]').each(function() {
                            var $this = $(this);
                            if ($this.is(':visible')) {
                        ...
                        $('.azh-edit-links div.azh-edit-link[style] a[href][style][target]').hide();
