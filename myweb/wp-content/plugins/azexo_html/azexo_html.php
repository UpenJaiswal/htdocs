...
function azh_get_object() {
    ...
    $azh = array(
        ...
        'shortcode_instances' => isset($settings['shortcodes']) ? $settings['shortcodes'] : array(),
        'site_url' => site_url(),
        'plugin_url' => AZH_URL,
        'ajaxurl' => admin_url('admin-ajax.php'),
        'helpers' => array(
        ...
        'fi18n' => array(
            ...
            'dropdown_options_edit' => esc_html__('Dropdown options edit', 'azh'),
        ),
        'i18n' => array(
    ...
    return apply_filters('azh_get_object', $azh);
}
...
function azh_add_meta_boxes($post_type, $post) {
    ...
    if (is_array($dirs)) {
        foreach ($dirs as $dir => $uri) {
            if (is_dir($dir) && in_array($post_type, apply_filters('azh_meta_box_post_types', array('page', 'azh_widget')))) {
                add_meta_box('azh', __('AZEXO HTML', 'azh'), 'azh_meta_box', $post_type, 'side', 'default');
                break;
            }
        }
    }
    if (in_array($post_type, apply_filters('azh_meta_box_post_types', array('page', 'azh_widget')))) {
        azh_editor_scripts();
...
function azh_page_row_actions($actions, $post) {
    if (get_post_meta($post->ID, 'azh', true)) {
        $actions['azh'] = '<a href="' . add_query_arg('azh', 'customize', get_permalink($post)) . '" title="' . esc_attr__('Edit page in frontend', 'azh') . '">' . esc_html__('Edit page in frontend', 'azh') . '</a>';
    }
    return $actions;
}
...
function azh_get_library() {
    ...
    return apply_filters('azh_get_library', array(
        ...
        foreach ($html->find('form') as $form) {
            ...
            if (count($form->find('input[name="form_title"]')) > 0) {
                ...
                foreach ($form->find('[name]') as $field) {
                    $fields[trim($field->name)] = array(
                        'tag' => $field->tag,
                        'label' => isset($field->placeholder) ? $field->placeholder : $field->name,
                    ...
                    if ($field->tag == 'select') {
                        ...
                        $fields[trim($field->name)]['options'] = $options;
                    }
                }
    ...
    if ($user_id) {
        ...
        if (!empty($pages)) {
            foreach ($pages as $page) {
                $forms = array_merge($forms, azh_get_forms_from_page($page));
            }
        }
    }
    return $forms;
        ...
        if ($user_id) {
            ...
            if ($post->post_author == $user_id) {
                ...
                if (isset($_POST['meta']) && is_array($_POST['meta'])) {
                    $post_meta = $_POST['meta'];
                    foreach ($post_meta as $key => $value) {
                        update_post_meta($_POST['post']['ID'], $key, $value);
                    }
                }
            }
        }
