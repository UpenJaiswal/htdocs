...
class Alg_WC_Cost_of_Goods_Bulk_Edit_Tool {
	...
	function enqueue_scripts_and_styles( $hook ) {
		...
		wp_enqueue_style( 'alg-wc-cog-bulk-edit-tool-style',
			alg_wc_cog()->plugin_url() . '/includes/tools/css/alg-wc-cog-bulk-edit-tool.css',
			array(),
			alg_wc_cog()->version
		);
		wp_enqueue_script( 'alg-wc-cog-bulk-edit-tool',
			alg_wc_cog()->plugin_url() . '/includes/tools/js/alg-wc-cog-bulk-edit-tool.js',
			array( 'jquery' ),
			alg_wc_cog()->version,
			true
		);
	 ...
	 * @todo    [dev] (maybe) better description here and in settings
