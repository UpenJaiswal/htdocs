...
function WooCommerce_MagicZoomPlus_send_stat($action = '') {
    ...
    $path = "api/stat/?action={$action}&tool_name=magiczoomplus&license=trial&tool_version=v5.3.2&module_version=v6.8.10&platform_name=woocommerce&platform_version={$platformVersion}&url={$url}";
    ...
    if (metadata_exists( 'post', $post_id, '_magic360_data' ) && magictoolbox_WooCommerce_MagicZoomPlus_check_plugin_active('magic360') ){
        ...
        if(!empty($magic360_plugin) && $magic360_plugin->params->checkValue('page-status','Yes') ){
            ...
            if(!empty($magic360_image_gallery)){
                ...
                if (!$useWpImages) {
                    $magic360_selector = '<a data-product-id="'.$post_id.'" data-magic-slide-id="360" style="display:inline-block;" class="m360-selector" title="360" href="#" onclick="return false;"><img src="'.WooCommerce_MagicZoomPlus_get_product_image($magic360_selector_path,'selector').'" alt="360" /></a>';
                } else {
